...
void Socket::setBlockingMode(BlockingMode newMode, std::error_code& errorCode) {
	...
	if (newMode == BlockingMode::UNSPECIFIED) {
		return;
	}

#ifdef _WIN32
	u_long nonBlock = (newMode == BlockingMode::NON_BLOCKING) ? 1 : 0;
	if (ioctlsocket(handle.get(), FIONBIO, &nonBlock) != 0) {
		errorCode = networking::Error::getLastErrorCode();
		return;
	}
#else
	int flags = fcntl(handle.get(), F_GETFL);
	if (newMode == BlockingMode::NON_BLOCKING) {
		flags |= O_NONBLOCK;
	} else {
		flags &= ~O_NONBLOCK;
	}
...
void Socket::setReceiveTimeout(Duration newTimeout, std::error_code& errorCode) {
	...
	const DWORD receiveTimeout = duration_cast<DWORDMilliseconds>(newTimeout).count();
#else
	...
	const TimevalSeconds timeoutSeconds = floor<TimevalSeconds>(newTimeout);
	const TimevalMicroseconds timeoutMicroseconds = duration_cast<TimevalMicroseconds>(newTimeout - timeoutSeconds);
...
#ifdef _WIN32
	using DWORDMilliseconds = DurationBase<DWORD, Ratio<1, 1'000>>;
	const DWORD sendTimeout = duration_cast<DWORDMilliseconds>(newTimeout).count();
#else
	timeval sendTimeout{};
...
Optional<Span<byte>> Socket::receive(Span<byte> buffer, MessageFlags flags, std::error_code& errorCode) {
	...
	const auto bytesReceived = recv(handle.get(), reinterpret_cast<char*>(buffer.data()), size, static_cast<int>(flags));
	if (bytesReceived < 0) {
		errorCode = networking::Error::getLastErrorCode();
		return {};
	}
...
Optional<Pair<Span<byte>, Endpoint>> Socket::receiveFrom(Span<byte> buffer, MessageFlags flags, std::error_code& errorCode) {
	...
	sockaddr_storage storage{};
	socklen_t length = static_cast<socklen_t>(sizeof(storage));
	const auto bytesReceived = recvfrom(handle.get(), reinterpret_cast<char*>(buffer.data()), size, static_cast<int>(flags), reinterpret_cast<sockaddr*>(&storage), &length);
	if (bytesReceived < 0) {
		errorCode = networking::Error::getLastErrorCode();
		return {};
	}
...
Optional<size_t> Socket::send(Span<const byte> bytes, MessageFlags flags, std::error_code& errorCode) {
	...
	const auto bytesSent = ::send(handle.get(), reinterpret_cast<const char*>(bytes.data()), size, static_cast<int>(flags));
	if (bytesSent < 0) {
		errorCode = networking::Error::getLastErrorCode();
		return {};
	}
...
Optional<size_t> Socket::sendTo(const Endpoint& endpoint, Span<const byte> bytes, MessageFlags flags, std::error_code& errorCode) {
	...
	if (sendto(handle.get(), reinterpret_cast<const char*>(bytes.data()), size, static_cast<int>(flags), reinterpret_cast<const sockaddr*>(endpoint.get()), endpoint.getLength()) <
		0) {
		errorCode = networking::Error::getLastErrorCode();
		return {};
	}
